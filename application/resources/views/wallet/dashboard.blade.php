    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="col-span-1 bg-white p-4 rounded shadow">
            ...
            <form action="{{ route('wallet.deposit') }}" method="POST" class="mt-4" {{-- hx-post="{{ route('wallet.deposit') }}"
                ...
                <x-form-input label="{{ __('messages.deposit_amount') }}" name="amount" focusLabel="deposit_amount" type="number" step="0.01"
                    :value="old('amount')" errorBag="deposit" />

                <x-form-input label="{{ __('messages.notes') }}" name="notes" focusLabel="deposit_notes" type="text" :value="old('notes')"
                    errorBag="deposit" />

                <button class="bg-green-500 text-white px-4 py-2 rounded mt-3">{{ __('messages.deposit') }}</button>
            </form>
        </div>
        ...
        <div class="col-span-2 bg-white p-4 rounded shadow">
            ...
            hx-target="body" hx-swap="innerHTML" --}}>
                ...
                <x-form-select label="{{ __('messages.receiver') }}" name="receiver_id" focusLabel="transfer_receiver_id" :options="$listItems"
                    :value="old('receiver_id')" placeholder="{{ __('messages.select_user') }}" required />

                <x-form-input label="{{ __('messages.amount') }}" name="amount" focusLabel="transfer_amount" type="number" step="0.01"
                    required :value="old('amount')" errorBag="transfer" />

                <x-form-input label="{{ __('messages.notes') }}" name="notes" focusLabel="transfer_notes" type="text" :value="old('notes')"
                    errorBag="transfer" />

                <button class="bg-blue-500 text-white px-4 py-2 rounded mt-3">{{ __('messages.transfer') }}</button>
            </form>
